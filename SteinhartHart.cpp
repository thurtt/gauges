...
double SteinhartHart::getTempKelvin(double resistance) 
{
	// double adc_raw = analogRead(_reading_pin);
	// double voltage = adc_raw / 1024 * V_IN;
	// double resistance = ((1024 * _resistance / adc_raw) - _resistance);
...
double SteinhartHart::getTempCelsius(double resistance) 
{
	return getTempKelvin(resistance) - 273.15;
}


double SteinhartHart::getTempFahrenheit(double resistance)
{
	return getTempCelsius(resistance) * 9/5 + 32;
}
